...
\label{figsolU}
\end{figure}
Now we proceed to estimate the value of $b$ that explains the best the data by using Bayes formula (\ref{eqnpropto}).
First we need to choose the prior probability for $b$. For the sake of the example, let us assume that 
equation (\ref{eqntoyproblem}) describes a well known physical process and it is known that
the parameter $b$ cannot be greater than $2$. In this case one way to choose the prior probablity
for $b$ that does not assume any other knowledge than $b\in (0,2]$ is the uniform distribution. 
Given a Borel measurable set $A\subset(0,2]$, the probability of $b$ belonging to $A$ is given by
\begin{equation*}
\frac{1}{2}\int_{A}dx.
\end{equation*}
In this case we have 
\begin{equation*}
\prior(b)=\frac{1}{2}\textbf{1}_{(0,2]},
\end{equation*}
...
we can solve the PDE for no more than 10 different values of $b$. The idea is to use GPs as described
in Chapter 2 to interpolate for the values of $b\in (0,2]$ where we do not solve equation (\ref{eqntoyproblem}). 
\newline
We need to choose 10 values of $b$, $\{b_{1},\dots,b_{10}\}$ to solve the PDE in a way that the interpolation error for 
the other values of $b$ in the set $(0,2]$ is small compared to the error in the interpolation if we were to choose
a different set of values. To choose the 10 points we use a maximin design as explained in Chapter 2. In this case
it is straightforward to check that the way to choose the points that maximize the minimum distance among them
is by locating them in an equidistant manner
\begin{equation*}
\{0.2,0.4,\ldots,2\}.
\end{equation*}
...
With this, we create the set of training points as $\{b_{i},u(\x_{k},b_{i})\}_{i=1}^{10}$ for each of the 
$k=10$ sites where we have the experimental measures $\{\tilde{u}(\x_{1};b),\ldots,\tilde{u}(\x_{10};b)\}$.
