...
With the concern for climate change and anthropogenic impact on the environment, such as 
airborne emissions of pollutants from industrial sites, have made
the importance of atmospheric dispersion models to grown over the years. Unfortunately,
atmospheric dispersion models are not  completely accurate \cite{chatwin1982use,lewellen1989meteorological}. 
...
approximate computationally demanding models with computationally cheap surrogates. 
...
The problem of estimating the source strengths from deposition measurements
...
approaches have been proposed in the literature. Lin and Chang \cite{lin2002relative} used an air
...
At this point we mention that when we refer uncertainty we are talking about every possible 
...
measurements of  zinc depositions at nine different locations as well as wind field velocity data 
...
We now turn our attention to the source density. We assume point-wise emission sources. Suppose there are $n$ sources 
...
Following \cite{monin1954basic},  the vertical diffusivity coefficient (in the $z$ direction) $D_{zz}$ is represented by the following expression
\begin{equation}\label{eqnEddyVertical}
D_{zz}=\frac{\kappa v_{*} z}{\phi(z/L)},
\end{equation}

\noindent where $\kappa$ is the \textit{Von-Karman} constant whose value in practice is set as $0.4$. The denominator
...
\noindent where the RHS is the $i$-th column of $A$. This result shows why
...
the parameter $\lambda$ will be specified  later in the chapter. 
...
\noindent In Figure \ref{figHistograms} we show the values of the point
...
the sense that Sources $1$ and $2$ are overestimated by the engineers. Emissions in source one are overstimated
by 12 and 20 tons per year respectively, whereas for Sources $3$ and $4$ are underestimated by 3 tons each (see Table \ref{tabLushi}). 
...
\label{tabHosseini}
\end{table}
When comparing the results in Figure \ref{figQUnicertainty} with the results obtained in \cite{hosseini2016airborne}, listed in Table \ref{tabHosseini}, we see that their 
...
computationally expensive atmospheric dispersion models. Also how to combine
...
done empirically with little or no use of experimental data. In this work we use
...
We stress out the conclusion that the methodology explained in this thesis goes beyond atmospheric dispersion models
...
dispersion models but to a more general setting.
