...
The foundations of the strategy to estimate parameters and solve inverse problems is lying in the 
...
with the mathematical language is the so-called Bayes formula \cite{jaynes2003probability}
...
know the coordinates where the rock hit the windows (finding the posterior), we need to assume that we know
where was actually the rock thrown and estimate how likely is that from that assumed position the rock 
would hit in the place where it did (finding the likelihood). For different supositions of where the rock
was thrown we need to weight that with how much do we  believe about the real position where was actually thrown 
(choosing the prior).
\newline
...
the kinematics equations of phisics for parabolic trajectories, i.e.
...
rock hit the window) assuming we know $r_{0}$ (where it was thrown). However the problem now complicates, since
...
that into account we add an extra layer to the model, we propose 
\begin{equation*}
...
of equation (\ref{eqnKinematics}). Introducing a random variable into the model makes all of the other varaibles
to behave as random variables too, therefore we can change the notation in equation (\ref{eqnBayes}) to 
the more suggestive
\begin{equation*}
P(\vec{r_{0}}|\vec{r})=\frac{P(\vec{r}|\vec{r_{0}})P(\vec{r_{0}})}{P(\vec{r})}.
\end{equation*}



The nice aspect about the way we introduce the  uncertainty term  is that \cite{Somersalo}
\begin{equation*}
\vec{r}|\vec{r_{0}},I\sim \mathscr{N}(\vec{r},\sigma I).
\end{equation*}

Now that we have a model for the likelihood, we now turn our attention to model the prior.

 In our hyphotetical
scenario where we are chilling at our place and a rock smashed our window. We have a very annoying neighbohr in 
front of our house. He is known for being a very aggresive person, so he is our main suspect. We have the hunch
...
\end{equation*}
where $\vec{x}$ is the coordinate of the center of mass of his room and $\lambda$ represent how much do we believe
the rock was thrown exactly from the point $\vec{x}$. This is one way to model the prior for our problem.

Finally for the normalization constant we choose $P(\vec{r})$ to be such that
\begin{equation*}
\int_{\mathbb{R}^{3}}P(\vec{r_{0}}|\vec{r})d\vec{r_{0}}=1\Rightarrow 
P(\vec{r})=\int_{\mathbb{R}^{3}}P(\vec{r}|\vec{r_{0}})P(\vec{r_{0}})d\vec{r_{0}}.
\end{equation*} 
...
the simluation we can finally evaluate the posterior $P(\vec{r_{0}}|\vec{r},I)$. Recalling that in general
...
Voil{\'a}! we solve our crime, we have a way to estimate where the rock was thrown and a way to measure how
confident we are about it. The posterior is what is called a solution when dealing with Bayesian inverse problems.
\newline
...
\item The dimensionality of the probabilities involve is high so sampling from them is far from trivial.
...
introduction of this work, all of the issues mentioned above come into play. In this chapter we are
...
Physical models of the reality are usualy complex and when numerically simulated, are computationally intense.
...
epismic or aleatory \cite{kennedy2001bayesian}.
\newline
Besides uncertainty there is another very important concept: sensitivity. When looking at a  model's perfommance
it is critical to assess how changes in the input $x$ affect the output $y$. Therefore if we assess sensitivity
of the model we can assess uncertainty of the outputs. One more reason one might be interest in performing
a sensitivity analysis is to detect what variables are relevant and what variables not, allowing to reduce
the dimensionality of the problem .

As mentioned in the introduction the physical model we are dealing with is complex and computationally intense.
...
\newline
Here is where the concept of emulator as defined in \cite{o2006bayesian} comes into play. The idea is to 
approximate the function $f(\cdot)$ with an approximation function $\hat{f}(\cdot)$. But the approximation
is not any approximation, is what we call and statitical approximation. The idea of this statistical 
...
\textbf{Be careful here, change this because was explicitly taken from the paper}
\begin{itemize}
\item At a design point $x_{i}$, the emulator should reflect the fact that we know the true value
of the simulator output, so it should return $\hat{f}(x_{i})=y_{i}$ without uncertainty.
\item At other points, the distribution for $f(x)$ should give a mean value $\hat{f}(x)$ that 
represents a plausible interpolation or extrapolation of the training data, and the probability 
distribution around this mean should be a realistic expression of uncertainty about how the 
simulator might interpolate/extrapolate.
\end{itemize} 
...
extrapolation/interpolation properties is what is known a Gaussian process (GP). 
...
To understand this notation consider the following case. We are trying to approximate a function $f(\cdot)$, then
the random variable $f(x)$ represents the value of the function $f(\cdot)$ at $x$. 
...
the covariance function is with the aid of some real valued function. A very common covariance
...
are higly correlated whereas far away points has a correlation that decays exponentialy fast with distance.
There are some `Standard' ways to choose the covariance function depending on the kind
of regularity we want for thr GP fitting. Some of the most common kernels are (setting $r=\|x-x'\|_{2}$
...
As we can see covariance kernel are defined in terms of parameters so depending on the model
under study we can find the paramters that best suits the model. The question now is: how
to choose the right parameters for the model? To answer this question we must talk about 
Bayesian model selecion.




Once the covariance function has been specified we end up with a distribution over functions. Talking 
about distribution over spaces of functions is a delicate topic that has to be done in a rigurous manner.
That is why at this point it is convenient to digress for a moment an talk about distributions in 
function spaces. The details presented here follows from \cite{lifshits2012lectures}.
...
normed vector spaces, they have a topology inhereted from the metric induced by the norm, therefore 
...
hand we  define a random vector taking values in $X$ as a measurable map 
\begin{equation*}
X:(\Omega,\mathscr{F},P)\longrightarrow\tvs,
\end{equation*}
where $(\Omega,\mathscr{F},P)$ is a probability space. To say that 
a random variable takes vales in $\tvs$ we will write $X\in\tvs$. 
...
In a more   general (infinite dimensional) famework we say that a vector $a\in\tvs$ is the 
...
With the construction of a rigurous framework that allows us to talk about Gaussian distributions in 
function spaces we can see why once the covariance function and the mean function is defined then we 
have created a Gaussian distribution over some function of spaces, the nature of the function
...
$T\subset\mathbb{R}^{n}$ where $T$ is compact. The space of real valued continuous functions in $T$ is a Banach
...
\end{equation*}
Therefore having a GP,  $\{f(x)\}_{x\in T}$ (see definition \ref{dfnGP}) with mean function $m(x)$ and 
covariance function $k(x,x')$, then we can see $f\in\tvs$ as a Gaussian random variable with mean
...
Now that we have a rigurous mathematical framework to work with GP, it is possible to explain how
they work. Imagine that we have some experimental measurements (train inputs) 
...
\end{eqnarray*}
Have a  joinltly Gaussian distribution given by  (assuming mean 0).
\begin{equation*}
\begin{bmatrix}
\textbf{f} \\
\textbf{f}_{*}
...
of $\textbf{f}_{*}|\textbf{f}$. Also we can have a good idea of how realiable if the fitting
by looking at the covariance matrix.
In the next chapter
we are going to show how to use GPs in practice. 
\newline
...
in a way that we can fill as much space as possible with as few training points as possible.

 This can be accomplish
through some space filling design. 
\newline


\subsection{Experimental Design and Sensitivity Analysis}

At this point it is clear that if we want to use GPs to fit the data obtained throught an emulator 
...
to choose as well. This is a very delicate issue since we have to find the right balance between
...
fitting in the output on all  every test point we choose.
\newline
 
Given an set $S$, there are several ways to create space filling designs 
\cite{pronzato2012design}. In this work we are going to focus on maximin designs
\cite{johnson1990minimax}. To define this kind of design, consider a set $T$ (e.g.
$T\subset\mathbb{R}^{n}$, compact) and any subset of $T$, $S$ with finite (fixed) cardinality, say $|S|=n$.
...
the points that live in each of the 8 vertex of the cube. In this case $d^{o}=1$. The prolbem of finding
...
The conexion of maximin experimental designs with GP is the following. Consider the
...
, then $det(K_{s})>0$. By minimizing the  negative of the determinant  we are gettin looking
for the maximum value of the determinat. This is achieved when the column vectors of a matrix
...
If we have a good space filling design, then we have a good fit when using GPs to interpolate in the 
points we want to get information about. If we have a realiable fitting we have the chance
to reduce the dimensionality of the model and hence its complexity. For instance assume that
you fit a function  $f:T\subset \mathbb{R}^{n}\longrightarrow\mathbb{R}$ and you want to 
know if all the $n$ variables in the domain are relevant to understand  the behaviour of $f$. For 
example if $f$ behaves like the function $f(x_{1},\ldots,x_{n})=x_{1}+x_{2}+10^{-8}x_{3}$ on $T=[0,1]^{3}$, 
then clearly 
...
sensitivity analysis \cite{saltelli2000sensitivity}. In this work we are going to focus on introduced
by Ilya Meyerovich Sobol. This method is what is called a variance-based Monte Carlo method (VBMCM).
The idea of a VBMCM is to use the variance of the inputs of a function as an indicator of its importance.
...
\end{equation*}
So by obtaining $f_{0}$ we can use recursion to obtain the other function in the decomposition. For example
...
behaviour of $f$ by defining on them its variances. The toal variance $D$ of $f$ is defined as
\begin{equation*}
D=\int_{\Omega^{n}}f^{2}(x)dx-f_{0}^{2}.
\end{equation*}
As we can see this is the same as the usual definition of the variance of a random variable $X$  in statistics as
\begin{equation*}
Var(X)=\E(X^{2})-\E(X)^{2}.
\end{equation*}
Where in this case the expectance operator is obtained by taking the integral over the whole space $\Omega^{n}$.
...
varibles $x_{i_{1}},x_{i_{2}},\ldots,x_{i_{s}}$. If 
...
fit using GPs or calculate sensitivity using Sobol indices accurately might be time consuming.
Thst's why as  we will see
in chapter four we used R packages to assit us with this kind of calculation. Next we are going to describe 
what R packages we used to get the results that are shown in this work.
...
,Matern $\frac{5}{2}$ and power exponential. It is also possible to work with taylored covariance
kernels but we won't explore that possibilty. Once the `km' object is created we can do 
...
The main function we used was the function SobolGP. This function takes as main  an input an object 
from the class `km' A matrix representing a sample of random points in the domain of the function $f$
we want to calculate it sensitivity (this function $f$ was previously 'fitted' by the function
...
chapter, so for chapter four we can focus on the results insted on how we apply what was explained here. 
