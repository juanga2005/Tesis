...
models and reality. It is then necessary to address to validity  of the model. One way to do
...
(training inputs) from an expensive function  $M(\cdot)$, 
, where $M(\textbf{x}_{i})=y_{i}$. For
simplicity we assume no trend in the training outputs $\{y_{i}\}_{i=1}^{m}$. Given the training data
we would like to infer  possible values of $M(\cdot)$ on another set of points 
$\{\textbf{x}_{j}^{*}\}_{j=1}^{k}$ (test inputs).
For this purpose we construct an emulator $\hat{M}(\cdot)$ (see introduction to section 2.1).
To construct $\hat{M}(\cdot)$  we  start considering the GP denoted by $\{f(\textbf{x})\}_{\x\in dom(M)}$
 where $dom(M)$ 
...
\right),
\end{equation}	
The zero mean models the assumption of no trend in the training output $\{y_{i}\}_{i=1}^{m}$.
Here
...
K(\x_{j},X)^{-1}K(X,X)\textbf{f}=y_{j}.
\end{equation} 
In this case,  the covariance matrix would 
...
$\widehat{M}(\x^{*}):=\langle f\rangle(\x^{*})$ then $\widehat{M}(\cdot)$ satisfies the 
...
of kernels and how to choose them depending the smooth properties we are looking
for in the interpolation process. 
...
To understand the role of the covariance kernel in the continuity of the mean function, let us
define some concepts first. ,
\begin{definition}
Let $\y,\x_{1},\x_{2},\ldots$ be a sequence of points in $\mathbb{R}^{n}$, such that 
\begin{equation*}
\|\x_{n}-\y\|\rightarrow 0\qquad\text{as }n\rightarrow\infty.
\end{equation*}
Then the collection of real valued random variables $\{f(\x)\}$ are say to be continuous in the mean
...
\end{equation*}
Therefore the continuity and differentibility properties of the mean function in a Gaussian process
...
Clearly this choice has a profound impact impact in the quality of the emulator.
Let us assume that the function $M(\cdot)$ is supported in $[0,1]$ and we  have
