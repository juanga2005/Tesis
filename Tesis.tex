...
%With $500.000$ samples from the posterior $\post(\pars,\q|\y)$ we are now 
...
it is clear that the  marginal posterior for the four sources is skewed and with a well-defined maximum.
...
First, the integral involved is highly dimensional,
hence is necessary to use Monte Carlo integration.  
...
to end up adding numbers that are, in general, smaller than machine epsilon. Hence, numerically
...
routine will not return an accurate value. Instead of using an optimization routine, 
we will use the fact that the marginal 
posterior for the sources is close to a gamma distribution. In this case, we 
...
In this case we can choose the  choose the  mode as a point estimate.
...
estimates for each distribution and the fitting curves for the parameter $p$
...
for a probability density $\rho$, the $68\%$ Bayesian confidence interval of that estimate, 
...
\hline 
\hline 
$p$ & Mode & 0.3774 & $[3,4]$\tabularnewline
\hline 
$z_{0}$ & Median & 0.0886 & $[0,2]$\tabularnewline
\hline 
$L$ & Median & -357.6 & $[-400,-200]$\tabularnewline
\hline 
$q_{1}$ & Mode & 23.41 & $[30,40]$\tabularnewline
\hline 
$q_{2}$ & Mode & 45.54 & $[70,90]$\tabularnewline
\hline 
$q_{3}$ & Mode & 58.67 & $[3,7]$\tabularnewline
\hline 
$q_{4}$ & Mode & 53.79 & $[3,7]$\tabularnewline
\hline 
\end{tabular}
\caption{Parameters and their estimates}
