...
atmospheric dispersion model's are not  completely accurate \cite{chatwin1982use,lewellen1989meteorological}. 
...
in measurement of the relevant variables.
...
\item Data uncertainty: experimental measurements or parameter estimation is inaccurate.
...
The reason of computationally efficiency is because atmospheric models are usually expressed as systems
...
use computationally demanding models in a cheap fashion. 

The mathematical model for atmospheric dispersion that we are 
interested is the advection-diffusion equation
for the concentration $C$ of particulate matter (particulate zinc is going to be our main focus
...
we are interested in modeling, we have four zinc sources at known locations $\{\x_{j}\}_{j=1}^{4}$ 
with strengths $\{q_{j}\}_{j=1}^{4}$ respectively. The values of the wind velocity field $\bar{\vv}$
and the diffusivity tensor $\textbf{D}$ depend upon five parameters. These parameters are going
to be explained in detail in Chapter 4. For the moment it is enough to consider these
parameters to be represented in a five dimensional vector denoted by  $\Theta$.
By replacing the linear operator $\dv(\vv+\textbf{D}\nabla)$ on the LHS by $L(\Theta)$, to make the dependence on $\Theta$ explicit, we can write equation \eqref{eqnModelIntro}
more compactly as
\begin{equation}\label{eqnParamtric}
\frac{\partial C(\x,t)}{\partial t}+L(\Theta)C(\x,t)=\sum_{j=1}^{4}q_{j}\delta(\x-\x_{j}).
\end{equation}

In practice, the concentration of particulate matter is not measured directly,
...
In this work we are mainly concerned  in estimating the sources' strength
given a finite set of deposition measurement at different locations.
The problem of estimating the sources' strength from deposition measurements
is known as source inversion.
More precisely, in this work, we want to estimate the source strength vector (or just sources for short)
\begin{equation*}
...
approaches have been envisioned in the literature. Lin and Chang \cite{lin2002relative} used an air
...
and sources. 
Getting ahead of the explanation of Bayes' rule in Chapter 2, we can formally the posterior distribution can be written as
\begin{equation*}
\post(\Theta,\q|\y)=\frac{\like(\y|\Theta,\q)\prior(\Theta,\q)}{Z(\y)},
\end{equation*}
where $\like$ is called the likelihood distribution, $\prior$ the prior
distribution and $Z(\y)$ is a normalization constant.


Traditionally, the process of tuning the parameters in atmospheric dispersion models has been done
empirically using Pasquill stability classes \cite{turner1994workbook, seinfeld1998atmospheric}.
...
of the simulation to other combinations of parameters and sources. We then use
Markov Chain Monte Carlo Methods to sample  the posterior distribution.
...
operated by Teck Industries Ltd.  We consider four smokestacks. Our ultimate goal
is to estimate the contribution of each stack  to the total amount of zinc
that is being released by the smelter. We have access to
measurements of  zinc depositions at nine different locations as well as field wind velocity data 
...
the physics of processes of interest. Here, it is necessary to develop, such
...
particulate zinc in the atmosphere. Consider a closed region  $\Lambda\subset\mathbb{R}^{3}$ with $m$ units of   mass
of zinc within it. 
Assume
...
at time $t$. The vector $\textbf{n}$ is the unit normal vector to $\partial\Lambda$ and $dA$ is an area element of it.
 On the other hand
...
where $dV$ is an element of volume in $\Lambda$. Finally, the amount of zinc that comes from the source at a time $t$ is given by
\begin{equation*}
...
necessary to put a minus in front of the surface integral for consistency. Assuming that
the concentration and velocity field are continuous functions of time and space, an 
straightforward application of the divergence theorem and Leibniz rule for integrals gives
\begin{equation*}
...
will find that the prediction for the concentration is not completely accurate, even if the model is completely accurate. 
...
c(\x,t):=\E(c)(\x,t)+c(\x,t)',
\end{equation}
where $c'$ satisfies $\E(c(\x,t)')=0$. The intuition behind this definition is the following: if we 
...
By plugging   equation (\ref{eqnNewC}) into equation (\ref{eqnNewRandom}) we obtain
...
We now turn our attention to the source density. W we assume point-wise emission sources. Suppose there are $n$ sources 
 at the points $\x_{1},\x_{2},\ldots,\x_{n}$,
...
The function $\delta(\cdot)$ is the Dirac delta function. For the case study of this chapter, we have $n=4$ sources 
...
Following \cite{monin1954basic},  the vertical diffusivity coefficient (in the $z$ direction) $D_{zz}$ is modelled by the following expression
...
the \textit{friction velocity} and modelled by
...
that the terrain of interest is regular, hence no significant variations of the wind velocity at a given height are 
expected.
In equation \eqref{eqn2Dvel}, $v_{set}$ is a constant given by  the settling velocity of the zinc particles. By Stoke's law,
...
$\mu$ is the viscosity of air. For the $x,y$ components of $\vv$ we assume a power law relation of the form
...
Hence, following \cite{hosseini2016airborne} we assume the diffusivity coefficient to be non-zero below a \textit{cutting height} $z_{cut}$.
...
over a different one or it is necessary years of experience from an expert to make a coherent sound choice. 
...
\end{equation}
Here $A$ is a $9\times 4$ matrix whose coefficients are the $a_{ij}(p,z_{0},z_{i},L,z_{cut})$.
\newline
...
of the parameters $(p,z_{0},z_{i},L,z_{cut})$ as we want and get a good
...
for a number of different values of $(p,z_{0},z_{i},L,z_{cut})$ and construct and approximation to $a_{ij}$
using these limited runs. We shall use the use Gaussian processes approach of Section \ref{secGPs}
to construct such approximation. Before we construct the
Gaussian processes we perform a sensitivity analysis as described in Section \ref{subsecSensitivity} to see if is possible
to reduce the dimensionality of the parameter space in order to further reduce to cost of constructing the Gaussian processes. 
...
\hline 
\end{tabular}
\caption{Model of interest to out sensitivity  study and their accepted ranges}
\label{tabRanges}
\end{table}
...
unit hypercube. Thus, without lost of generality we may assume the deposition maps are of the form 
\begin{equation*}
...
$\{\varphi_{x_{i},y_{i}}\}_{i=1}^{9}$ at a large number of points, which can be expensive.
...
\end{figure}
Figure $\ref{figSensitivityPlot}$ shows that two of the five variables, namely $z_{i}$ and $z_{cut}$, have very little influence on the variance 
...
We now turn our attention in how to find an approximation for the 
matrix $A(p,L,z_{0})$.
\newline
...
volume code on a $30\times 30\times 30$ grid takes from 30 minutes and up to a few hours. Hence the 
...
With the experimental design at hand, we run the finite volume solver 
for each of the different 64 configurations of parameters. We repeat this step four
...
With the emulator at hand, we are now in position to apply
...
\item Engineering estimates of emission rates are as follows (These values are taken as a guideline).
\begin{table}[H]
\centering
\begin{tabular}{|c|c|}
\hline 
...
\end{table}
\end{enumerate}
Mathematically, (1) above can be modelled with an independent  prior:
\begin{equation*}
...
engineers estimate \cite{dudley2002real}. 
...
To make notation simpler we have chosen to represent the vector $[q_{1},\ldots,q_{4}]^{T}$ as $\q$ or as component-wise in the summation. The indicator function represents the ranges of the allowed  values of the parameters in Table (\ref{tabRanges}). 
...
types of point estimates, such as Maximum a posterior
or posteri or conditional mean (see equation $\eqref{eqnpointestimates}$). To find any of these estimates   and its uncertainty associated,  is necessary to perform  high
...
$\lambda$ that weights the importance of the likelihood correctly. That is,
...
Estimating $J(\lambda)$ exactly is expensive, since  it involves evaluating a seven-dimensional
integral. Thus, to approximate a minimizer for $\lambda$  we will approximate $J$ using Gaussian processes emulator. 
...
Since the acceptance rate was relatively low, around $10\%$, we thined the last million samples
by picking a sample every hundred samples. In this way we end up with $10.000$ samples that are
weakly correlated in the sense that the autocorrelation function is close to zero.
\\
Using MH to sample
from high-dimensional probability distributions is challenging. Specially assessing the convergence of the Markov chain
...
One of the advantages of every sampling algorithm is that the samples
...
In Figure \ref{figHistograms} we show the values of the point
...
estimates and the uncertainty associated.
...
Thus, finding this mode is a reasonable choice for a point estimate. To approximate the 
value of the mode in the marginal of the sources, we are going to take
advantage of the fact that the histograms for the marginals have a similar
shape to a gamma distribution and we are going to a
 gamma distribution  to the histograms and
take the point estimate to be the mode of the fitted distribution.
...
the distribution has no distinctive points. This means the data has little information
regarding the values of $p$. For  $z_{0}$ and $L$
there are values that are more distinctive than others, but is not a very sharp
distinction. To pick a point estimate
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{./FigChap4/histogramsII}
...
of the parameters show that the experimental data is not large enough
...
probability values does not agree with the typical
...
\caption{Comparison between engineers source strength estimation with 
our point estimate and the uncertainty associated to it.}
\label{figQUnicertainty}
\end{figure}
The results in Figure \ref{figQUnicertainty}, show that sources $1$ and $2$ are most likely overestimated by the engineers.
Whereas sources $3$ and $4$ are  slightly underestimated. If we take into account the error bars for our estimates and
the small difference between that estimate and the engineers estimate, we conclude that the experimental data
...
we obtained  agree with \cite{lushi2010inverse} but in \cite{hosseini2016airborne}, source
$2$ is severally underestimated. 


The results obtained in Figure \ref{figQUnicertainty}, agree with the results obtained in \cite{lushi2010inverse} in 
the sense that sources $1$ and $2$ are overestimated by the engineers and sources $3$ and $4$ are underestimated. 
These results partially disagree with the results in \cite{hosseini2016airborne}, in this reference, source $2$
is underestimated. In the other three sources our results agree.
