...
the cases when $\lambda$ is very small or $\lambda$ is very large. In the former case the exponential  term 
\begin{equation*}
\exp\left(-\frac{1}{2\lambda^{2}}\|\widehat{A}\q-\y\|^{2}\right),
\end{equation*}
in the likelihood becomes negligible unless $\|\widehat{A}\q-\y\|$ is of the order of $\lambda$. The interpretation
...
When $\lambda$ is large the exponential flattens out. In this scenario
...
\begin{equation*}
J(\lambda)=\frac{1}{2}\E_{\post^{\lambda}}\left[\|\widehat{A}(p,L,z_{0})\q-\y\|_{2}+\|\q-\q_{est}\|_{2}\right],
\end{equation*}
...
accurate so the engineers estimates about the sources can be dismissed. Hence, by choosing $\delta=\frac{1}{2}$
...
Estimating $J(\lambda)$ accurately is expensive, since  it is necessary to evaluate a seven-dimensional
...
Using the adaptative algorithm we obtained two million samples and  as a burn in period we discarded the first million  samples. 
...
Recall that the probability density we are interested, is the  posterior in equation (\ref{eqnPosteriorFinal}). 
This posterior is supported in a  subset of $\mathbb{R}^{7}$, thus to obtain the trace plots
of it, we need to plot the motion of the Markov chain in each of the seven dimensions. These plots
...
shape of a gamma distribution and we are going to fit over them
the gamma distribution that is closest to the histograms shape. We
take the point estimate to be the mode of the fit.
...
the distribution has no distinctive points. For  $z_{0}$ and $L$
there are values that are more distinctive than others, but is not a very sharp
distinction. To pick a point estimates
...
\hline 
\end{tabular}
\caption{Parameters and their estimates and uncertainties}
...
so that the posterior marginal distribution for the parameters is 
not decisively informative about a value or a particular set 
of values the parameters are likely to be. Despite this, 
as we can see in the histograms in Figure \ref{figHistograms}, for
$L$ and $z_{0}$ we can see that there are region in the support
...
to distinguish regions with high probability from
regions with low probability. For the value of $z_{0}$
...
In conclusion, we have developed a framework that allows us to  do a source inversion and obtain estimates
of unknown parameters, with the difference that we can quantify the uncertainty in those estimates, as opposed
to classical approaches to source inversion. \textbf{What to do in the future?}.
