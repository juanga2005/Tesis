...
\end{equation}
Note that finding the posterior enables us to   any point
...
\end{equation*}
such that  each $k$ and $b$,  the value of the mean of the GP is going to be given by $G_{k}(b)$. 
This value is the approximation of $u(\x_{k},b)$. Note that to train the GPs we
 have a training set for each point in $P$.
For example, for the point $\x_{k}$, the training set is $\{b_{i},u(\x_{k},b_{i})\}_{i=1}^{10}$.
...
the GP regression over the test points,  the true value of $b$,
and the experimental measurements $\tilde{u}(\x_{k};b)$ for each of the ten sites.
...
\textbf{After improving the graph, now you keep moving}
...
As mentioned in the introduction, we are interested to study
...
is to estimate how much is each source contributing to the total amount of zinc
...
experiment a large number of times under identical initial conditions.
...
With the mathematical model formulated we are ready to formulate the inverse problem.
