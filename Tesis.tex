...
Our starting point is to consider the set of  map $\varphi_{x,y}$ that takes  a set of parameters $(p,z_{0},z_{i},L,z_{cut})$
as inputs and gives the deposition value at the point $(x,y)$  as output. 
Following \cite{seinfeld1998atmospheric}, the accepted
...
unit hypercube $[0,1]^{5}$. Thus, we may assume the deposition maps are of the form 
\begin{equation*}
...
Sobol indices is necessary to perform numerical integrations over the integrand $\varphi_{x,y}$.
...
to know the output of $\varphi_{x,y}$ at any point in its domain. In general we do not
know the output of the function of interest beyond a very limited number of points. 
To extrapolate $\varphi_{x,y}$
...
Implementing the routines to  emulate $\varphi_{x,y}$ and estimate
...
\caption{Boxplots containing the result of for the total effect Sobol index performed on each of the nine sensors.
...
are held fix at the values 100 and 2 respectively (see \cite{hosseini2016airborne}).  
...
and use the results to obtain information about $A$. Solving equation (\ref{eqnDifussivityFinalForm})
is a computationally intensive task. Is not feasible to solve it for a wide range
of different values of $(p,L,z_{0})$ in order to obtain information about $A$. Instead, to 
learn about $A$ for a wide range of values of $(p,L,z_{0})$, we will use an emulator.
...
of equation (\ref{eqnDifussivityFinalForm}) and the physics.
\begin{figure}[H]
\centering
...
By having the emulator, we are now in position to apply
...
the values of the sources $q_{1},q_{2},q_{3},q_{4}$ are not. Thus, it is reasonable
...
parameters. Thus we assume all three parameters are independent to each other.
...
is parametric distribution and will be 
...
\end{equation*}
where $\alpha$ and $\beta$ are constants. We are going to assume that each source is distributed
with gamma probability density. In this case we write
\begin{equation*}
...
the engineer estimate for the $i$-th source, represented as $q_{eng,i}$.
More precisely we choose the values of $\alpha_{i}$ and $\beta_{i}$ 
as the solution of the 
following system of equations
\begin{eqnarray*}
\beta_{i}(\alpha_{i}-1)=q_{eng,i}, \\
...
\begin{equation*}
\max_{x\in [0,\infty)}f(q|\alpha_{i},\beta_{i})=q_{eng,i}\qquad\text{for }i=1,2,3,4.
\end{equation*}
and $99\%$ of the mass of the density is concentrated between 0 and 3 times the
engineers estimate. 
Combining the results from equations (\ref{eqnPriorq}) and (\ref{eqnPriorparam})
we conclude that the prior distribution  is proportional to
...
The ranges for the values of the parameters in Table (\ref{tabRanges}) are widely accepted in the literature. However there is not
a technically sounded reason of why these ranges are acceptable. Instead we will expand the possible values the parameters are 
allowed to take and test the validity of the values in Table (\ref{tabRanges}). The new set of ranges we picked are shown
...
To find  point estimates such as those  in equation (\ref{eqnpointestimates}) and its uncertainty associated,  is necessary to perform a high
...
that variable regardless of any other variable or parameter. The histograms
for the   marginals  of each of the variables of interest in the posterior are shown
in Figure \ref{figHistograms}.

With $500.000$ samples from $\post(\pars,\q|\y)$ we are now 
in position to perform Monte Carlo integration
in order to obtain point estimates and their associated uncertainty.
The point estimate we are going to calculate is the mean (see equation (\ref{eqnpointestimates})).

For example, for the parameter $p$, its mean is calculated via the approximation

\begin{equation*}
\E[p|\y]=\int_{0}^{0.6}p\p(p|\y)dp\approx\frac{1}{500000}\sum_{j=1}^{500000}\p(p_{j}|\y),
\end{equation*}
where $\p(p|\y)$ is the marginal distribution for $p$ and $p_{j}$ is the $j$-th sample obtained
from the MH algorithm. The estimate for the variance for the variable $p$ is calculated as
\begin{equation*}
\mathbb{V}[p|\y]=\int_{0}^{0.6}(p-\E[p|\y])^{2}\p(p|\y)dp\approx\frac{1}{500000}\sum_{j=1}^{500000}(p_{j}-\E[p|\y])^{2}.
\end{equation*}


The values for the mean and the variance for each of the variables of interest are shown in Table \ref{tabEstimates}.


\begin{table}[H]
\begin{tabular}{|c|c|c|c|}
\hline 
Parameter & Mean & Standard Deviation & Coeff. of Variation\tabularnewline
\hline 
\hline 
$p$ & 0.3058597117 & 0.134048244 & 0.4382670843\tabularnewline
\hline 
$z_{0}$ & 0.1337796345 & 0.1935939725 & 1.4471109388\tabularnewline
\hline 
$L$ & -345.0048422213 & 130.9004274299 & 0.3794162035\tabularnewline
\hline 
$q_{1}$ & 0.003816668 & 0.0037975963 & 0.9950030602\tabularnewline
\hline 
$q_{2}$ & 0.0040946827 & 0.0027761492 & 0.6779888475\tabularnewline
\hline 
$q_{3}$ & 0.0052248901 & 0.004251651 & 0.8137302262\tabularnewline
\hline 
$q_{4}$ & 0.0076034684 & 0.0058865728 & 0.7741957376\tabularnewline
\hline 
\end{tabular}
\caption{Estimates and the uncertainty associated to them.}
\label{tabEstimates}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[scale=0.7]{./FigChap4/histograms}
\caption{Histograms}
\label{figHistograms}
\end{figure}
\bibliography{Tesis}
