...
\thesistype{Thesis}
\author{Juan Gabriel Garc{\'i}a Osorio}
\previousdegrees{%
	B.Sc(Physics), National University of Colombia, 2013\\
	B.Sc(Mathematics), National University of Colombia, 2015}
...
\committee{%
	\chair{Razvan Fetecau}{Associate Professor}
	\member{John Stockie}{Senior Supervisor\\Professor}
	\member{Paul Tupper}{Committee\\Professor}
	\member{Ben Adcock}{Internal Examiner\\Assitant Professor}
}
...
mistake. First I would like to express my gratitude to the people that are closest to my life: my mom and my best friends 
...
with me, and the many hours and expertise he put into helping me assemble this thesis. Last but not least
a very special thank you to all my friends at PIMS and SFU that made my way through grad school  a total blast 
and all the professors from  whom I had the fortune to learn, not only academically but also how to be a better
person.
...
With raising concerns over  climate change and anthropogenic impact on the environment, such as 
...
in this case there is a limit to how accurate a model can be due to uncertainties
...
and the diffusivity tensor $\textbf{D}$ depend upon certain number of parameters. These parameters will
be explained in detail in Chapter 4. For the moment it is enough to consider these
parameters to be represented by a $k$-dimensional vector denoted by  $\Theta$.
...
perspective, this number can be interpreted as: in all multiverses similar to ours, on average, one universe
out of a hundred, shelters life.
 In the Bayesian
perspective the number 0.01 is interpreted as a measure of how certain we are about life in our universe
...
When we mention uncertainty we are talking about every possible 
...
\textbf{r}_{ML}&=&\argmax_{\textbf{r}_{0}}\like(\textbf{r}|\textbf{r}_{0},\textbf{v}_{0})
...
hard to sample from.
\item Evaluating any of the possible point estimates for the quantity of interest might  be computationally difficult.
\end{enumerate}
...
\begin{itemize}
\item At points $\{\x\}_{k=1}^{N}$  where we know the output of the mathematical model (i.e. we know 
...
\begin{equation*}
\E(f,X)=\langle f, x\rangle,\qquad\text{for all }f\in\tvs^{*}.
\end{equation*}
...
As an example consider  $\tvs=\mathbb{C}(T)$, the set of continuous real valued functions on $T$, where 
...
$(\Omega,\mathscr{F},\p)$ are said to be continuous at $y$ in the mean-squared sense if \cite{rasmussen2006gaussian} 
\begin{equation*}
...
\end{equation*}
whenever the limit exists. Here $\textbf{e}_{i}$ is the $i$-th canonical vector of the standard basis in $\mathbb{R}^{n}$.
...
In the definitions above if we take $\{f(\x)\}$ to be a Gaussian process with stationary covariance kernel $k$ then, it can be shown that 
...
\begin{equation*}
\max_{(S\subset T,\text{ }|S|=n)}\min_{(s,s'\in S,s\neq s')}d(s,s')=\min_{s,s'\in S^{o},s\neq s'}d(s,s')=d^{o}.
\end{equation*}
...
We use this set  to train ten Gaussian Processes. With these processes we define the functions 
\begin{equation*}
...
$\frac{\post(q_{2}|\y)}{\post(q_{1}|\y)}$ otherwise.  We choose
...
is the  graph for the posterior $\post(b|\y)$.}
\label{figToyMCMC}
\end{figure}
...
b_{cm}=\int_{(0,2]}b\post(b|\y)db\approx\frac{1}{5000}\sum_{j=1}^{5000}b_{j}=0.9247042,
\end{equation}
...
\begin{equation*}
\int_{(0,2]}(b-b_{cm})^{2}\post(b|\y)db\approx\frac{1}{5000}\sum_{j=1}^{5000}(b_{j}-b_{cm})^{2}=0.01427.
\end{equation*}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.75]{./FigChap4/sensitivityPlotPaper.pdf}
...
\section{Inferring  Parameters and  the Sources}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{./FigChap4/bar_plot_all_results}
\caption{Comparison between engineering estimates of the source strength source with 
our point estimate and the uncertainty associated to it. We also compare with the results 
obtained in \cite{hosseini2016airborne}(S $\&$ H) and \cite{lushi2010inverse}(S $\&$ L).}
