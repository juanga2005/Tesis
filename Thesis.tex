...
\previousdegrees{%
	B.Sc., National University of Colombia, 2013\\
	B.Sc., National University of Colombia, 2015}
...
\keywords{Bayesian Statistics, Gaussian Processes, Parameter Estimation,
Experimental Design, Model Emulation, Bayesian Inversion, Atmospheric Dispersion, Source Inversion.}

\committee{%
	\chair{TBD}{Professor}
	\member{John Stockie}{Senior Supervisor\\Professor}
	\member{Paul Tupper}{Internal Examiner\\Assistant Professor}
}
...
\begin{abstract}
In this thesis we present a method to obtain a computationally efficient way
...
We constructively explain step by step the theory and ideas behind each relevant part 
of the process in the emulation and parameter estimation. The concepts and methodology presented 
in this work are wide in scope and can be applied
to all types of problems where is necessary to obtain a parameter estimation  but the underlying
mathematical model is expensive, rendering unfeasible more classic approaches. 

To validate the concepts used, we do an emulation and a parameter estimation
in a model that is computationally cheap and whose parameter values are known in 
advanced. Finally we perform a parameter estimation with
uncertainty quantification of an atmospheric dispersion model
using real data from a mining site in Trail, British Columbia. 
The estimation includes solving the source inversion problem
